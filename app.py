...
# ======================================================
# 2. FUNÇÕES DE DADOS (ADAPTADO PARA SQLITE)
# ======================================================
# Conexão SQLITE
...
def carregar_estoque():
    try:
        # show_spinner=False remove o carregamento da tela
        df = conn.query("SELECT * FROM estoque", ttl=0, show_spinner=False)
        if not df.empty:
            df = df.set_index('Produto')
            return df
    except:
        # Se a tabela não existir, cria a inicial
        df_init = pd.DataFrame({
            'Quantidade': [100, 50], 
            'Preço unitário': [80.0, 110.0],
            'Custo unitário': [40.0, 55.0]
        }, index=['Camisa Oversized', 'Camisa Suedine'])
        df_init.index.name = 'Produto'
        salvar(df_init, "estoque", index=True)
        return df_init

def carregar_vendas():
    try:
        df = conn.query("SELECT * FROM vendas", ttl=0, show_spinner=False)
        if not df.empty:
...
def carregar_caixa():
    try:
        df = conn.query("SELECT * FROM caixa", ttl=0, show_spinner=False)
        if not df.empty: return df
    except: pass
    return pd.DataFrame(columns=['Data', 'Vendedor', 'Tipo', 'Descrição', 'Valor', 'Metodo'])

def salvar(df, tabela, index=False):
    # Usamos conn.engine para garantir a compatibilidade com o SQLite
    df_salvar = df.reset_index() if index else df
    df_salvar.to_sql(tabela, conn.engine, if_exists='replace', index=False)
       ...
       with col_recibo:
           ...
           if st.button("CONCLUIR VENDA", use_container_width=True):
               if df_estoque.loc[categoria, 'Quantidade'] >= qtd:
                   ...
                   salvar(df_estoque, "estoque", index=True)
...
# ======================================================
   ...
   with st.container(border=True):
       ...
       if st.button("Salvar Modificações"):
           salvar(df_edit, "estoque", index=True)
